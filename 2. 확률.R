...
#     통계적 확률은, 큰수의 법칙의 한 경우일 뿐이다. 통계적 확률 = 표본평균, 이다.
...
x = rbinom(100, 1, 0.5)
mean(x) # 0.52, 시행횟수를 늘렸더니 통계적확률이 수학적확률에 가까워졌다.



## 몬테카를로 시뮬레이션(방법) (Monte Carlo simulation(method)): 
#       수학공식 없이 확률을 계산하는 방법이다. 20c 도박의 도시 몬테카를로에서 이름을 땄다.
...
print(4 * (inner_n / sim_n)) # 3.136, 원주율 파이를 구했다.
